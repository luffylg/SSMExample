 ...
 * @类描述：DailyRollingFileAppender extendsFileAppender </br>
 *                               重写RollingFileAppender类 </br>
 *                               实现maxBackupIndex限制日志文件数量功能（定时删除超过N天前的日志文件）
 * @创建人：wyait
 * @创建时间：2017年8月8日下午2:40:38 @version：1.0.0
 */
public class CustomDailyRollingFileAppender extends FileAppender {
    ...
    /**
     * 有参构造 Instantiate a<code>DailyRollingFileAppender</code> and open the file designated by<code>filename</code>. The opened
     * filename will become the ouput destination for thisappender.
     */
    ...
    /**
     * 核心方法：生成日志文件，并完成对备份数量的监测以及历史日志的删除 Rollover the current file to a new file.
     */
     ...
     *
     * <p>
     * Before actually logging, thismethod will check whether it is time to do a rollover. If it is, it willschedule the next
    ...
    /**
     * 获取同一项目日志目录下所有文件 This method searches list of log files based on the pattern given in the log4jconfiguration file and
...
/**
 * 自定义file类，重写compareTo方法，比较文件的修改时间 The ClassModifiedTimeSortableFile extends java.io.File class and implements Comparable to
 * sortfiles list based upon their modified date
 */
...
/**
 * RollingCalendar is a helper class toDailyRollingFileAppender. Given a periodicity type and the currenttime, it computes the
